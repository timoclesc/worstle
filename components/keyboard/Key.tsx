...
const Key: FunctionComponent<KeyProps> = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`${width} ${bgColor} ${textColor} fill-current rounded text-[13.333px] font-bold select-none uppercase h-14 flex justify-center items-center transition-colors duration-200 delay-[1500ms]`}
    >
      {children}
    </button>
  );
